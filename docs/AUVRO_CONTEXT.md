...
- **Subdominio de Netlify con el slug (14 Ago)**: el site se crea con `name: slug` para que el subdominio quede `<slug>.netlify.app` en vez de un nombre aleatorio. Si ese nombre ya está tomado en Netlify (422), se reintenta sin `name` y Netlify asigna uno aleatorio.
